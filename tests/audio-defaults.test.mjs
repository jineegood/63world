...
test('the server stores the same defaults so the level cannot jump on login', () => {
  const api = loadApi();
  const migration = read('supabase/migrations/202607280001_default_audio_levels_v3.sql');
  assert.match(migration, new RegExp(`alter column bgm_volume set default ${api.BGM_PERCENT}\\b`, 'i'));
  assert.match(migration, new RegExp(`alter column sfx_volume set default ${api.SFX_PERCENT}\\b`, 'i'));
  // 직접 조절한 학생의 값까지 덮어쓰면 안 된다
  assert.match(migration, /where bgm_volume = 55\s*\n\s*and sfx_volume = 65/i);
});
