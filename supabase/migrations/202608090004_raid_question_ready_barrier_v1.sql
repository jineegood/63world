-- Start each raid question's 30-second timer only after all three independent
-- browsers have received and rendered that round.  The previous playback barrier
-- protected the end of a round, but the host could open the next question while a
-- guest was still showing its sync overlay.  That guest then lost most or all of
-- the answer time and was auto-submitted as wrong.

alter table public.raid_room_members_v1
  add column if not exists question_ready_round integer not null default 0
  check (question_ready_round >= 0);

-- Rooms already answering at deployment time used the old immediate deadline.
-- Mark those members ready so an in-flight class is not moved backwards.
update public.raid_room_members_v1 member
   set question_ready_round = greatest(member.question_ready_round, room.round_no)
  from public.raid_rooms_v1 room
 where room.id = member.room_id
   and member.active
   and room.phase in ('question', 'waiting', 'resolving')
   and room.question_deadline is not null;
...
begin
  ...
  update public.raid_rooms_v1 set
    ...
    -- Deliberately null until private_ack_raid_question_ready_v1 sees all three.
...
create or replace function public.private_ack_raid_question_ready_v1(
  p_user_id uuid,
  p_room_id uuid,
  p_round_no integer,
...
begin
  ...
  if v_room.round_no <> p_round_no then
    raise exception using errcode = 'P0001', message = 'ROUND_CHANGED';
  end if;
  if v_room.phase not in ('question', 'waiting') or v_room.question_public is null then
    raise exception using errcode = 'P0001', message = 'ROUND_CLOSED';
  end if;

  update public.raid_room_members_v1
     set question_ready_round = greatest(question_ready_round, p_round_no),
         last_seen_at = greatest(last_seen_at, p_ready_at)
  ...
  select count(*), count(*) filter (where question_ready_round >= p_round_no)
  ...
  if v_ready_count = v_member_count and v_room.question_deadline is null then
    update public.raid_rooms_v1
       set question_deadline = p_ready_at + interval '30 seconds',
           version = version + 1,
...
begin
  ...
  if v_room.question_deadline is null then
    raise exception using errcode = 'P0001', message = 'QUESTION_PENDING';
  end if;
  ...
  select count(*) into v_member_count
    from public.raid_room_members_v1 where room_id = p_room_id and active;
  select count(*) into v_input_count
    from public.raid_round_inputs_v1 where room_id = p_room_id and round_no = p_round_no;
  v_resolving := v_input_count >= v_member_count or v_room.question_deadline <= p_submitted_at;
...
revoke all on function public.private_ack_raid_question_ready_v1(uuid, uuid, integer, timestamptz)
  from public, anon, authenticated;
grant execute on function public.private_ack_raid_question_ready_v1(uuid, uuid, integer, timestamptz)
  to service_role;
...
revoke all on function public.private_submit_raid_round_v1(uuid, uuid, integer, text, text, text, timestamptz)
  from public, anon, authenticated;
grant execute on function public.private_begin_raid_round_v1(uuid, uuid, jsonb, text, text, timestamptz)
  to service_role;
grant execute on function public.private_submit_raid_round_v1(uuid, uuid, integer, text, text, text, timestamptz)
  to service_role;
