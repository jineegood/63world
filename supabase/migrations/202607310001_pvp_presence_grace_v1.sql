-- Keep the serialized invite/match flow while allowing for throttled classroom
-- browsers. A visible remote player disappears after six seconds, and invites
-- still expire after twenty seconds; this grace only prevents false OFFLINE
-- results when a background timer or Wi-Fi briefly delays presence refreshes.
begin;
...
begin
  ...
  select
    count(*)::integer,
    coalesce(bool_and(last_seen_at >= p_requested_at - interval '90 seconds'), false),
...
begin
  ...
  select
    count(*)::integer,
    coalesce(bool_and(last_seen_at >= p_accepted_at - interval '90 seconds'), false),
    coalesce(bool_and(map = 'town'), false),
    coalesce(bool_or(busy), true)
    into v_presence_count, v_all_online, v_all_in_town, v_any_busy
    from public.pvp_presence_v1
