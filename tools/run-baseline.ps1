param(
  [ValidateSet('all', 'check', 'baseline', 'core-utils', 'player-store', 'input-router', 'world-interaction-registry', 'world-navigation-registry', 'click-movement', 'click-movement-integration', 'world-render-pipeline', 'hud-update-pipeline', 'total-stats-pipeline', 'combat-entry-pipeline', 'combat-frame-pipeline', 'audio-volume-pipeline', 'audio-defaults', 'profile-portrait', 'combat-rules', 'combat-sequence-controller', 'combat-flow', 'combat-fx', 'combat-log-v3', 'authoritative-combat-presentation-v3', 'authoritative-combat-game-flow', 'server-combat-polish', 'sfx-map', 'audio-manifest', 'audio-dispatcher', 'weapon-tier', 'game-data', 'quest-data', 'quest-text', 'quest-dialogue-theme', 'workbook-import', 'chatgpt-prompt', 'patch-data', 'gameplay-polish-v2', 'early-game-polish-v2', 'wrong-answer-polish-v2', 'world-healing-polish-v2', 'reward-presentation-v2', 'tutorial-quests-polish-v2', 'tutorial-highlight-v1', 'login-keys', 'multiplayer', 'remote-motion', 'pvp-rules', 'pvp-policy-v1', 'pvp-function-v1', 'pvp-client', 'pvp-profile-ui', 'pvp-battle-ui', 'pvp-reconnect-v1', 'server-authority-v3-pvp', 'supabase-security-v2', 'server-authority-v3', 'authority-catalog-v3', 'combat-catalog-v3', 'quest-catalog-v3', 'server-authority-v3-quests', 'quest-game-wiring-v3', 'server-authority-v3-combat', 'pve-combat-rules-v3', 'pve-combat-service-v3', 'pve-combat-function-v3', 'authoritative-combat-escape-migration-v3', 'pve-combat-client-v3', 'pve-combat-game-wiring-v3', 'server-authority-v3-economy', 'authority-action-runner-v3', 'server-authority-v3-economy-wiring', 'player-authority-v3', 'server-authority-v3-game-wiring', 'cloud-sync-v2', 'student-access-v2', 'secure-student-login-v2', 'secure-shared-student-v2', 'admin-auth-v2', 'teacher-reset-function', 'teacher-reset-player-v3', 'secure-teacher-auth-v2', 'secure-shared-teacher-v2', 'admin-data-v2', 'student-reward-grants-v2', 'teacher-delete-function', 'secure-cloud-student-admin-v2', 'shared-state-policy-v2', 'shared-state-v2', 'refactor-health', 'current-data', 'safety-net', 'extract-data', 'build-workbook')]
  [string]$Mode = 'all'
)
...
if ($Mode -eq 'all' -or $Mode -eq 'check') {
  ...
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--check', 'src/combat-log-v3.js')
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--check', 'src/authoritative-combat-presentation-v3.js')
...
if ($Mode -eq 'all' -or $Mode -eq 'audio-defaults') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/audio-defaults.test.mjs')
}

if ($Mode -eq 'all' -or $Mode -eq 'profile-portrait') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/profile-portrait.test.mjs')
}
...
if ($Mode -eq 'all' -or $Mode -eq 'combat-log-v3') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/combat-log-v3.test.mjs')
}

if ($Mode -eq 'all' -or $Mode -eq 'authoritative-combat-presentation-v3') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/authoritative-combat-presentation-v3.test.mjs')
}

if ($Mode -eq 'all' -or $Mode -eq 'authoritative-combat-game-flow') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/authoritative-combat-game-flow.test.mjs')
}

if ($Mode -eq 'all' -or $Mode -eq 'server-combat-polish') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/server-combat-polish.test.mjs')
}
...
if ($Mode -eq 'all' -or $Mode -eq 'authoritative-combat-escape-migration-v3') {
  Invoke-CheckedNode -NodeExe $nodeExe -Arguments @('--test', 'tests/authoritative-combat-escape-migration-v3.test.mjs')
}
